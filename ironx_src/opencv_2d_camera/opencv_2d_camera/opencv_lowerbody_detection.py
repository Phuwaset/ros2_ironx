...
class ImagePublisher(Node):
    ...
    def __init__(self):
        ...
        self.vdo_lowerbody_detection_publisher = self.create_publisher(Image, 'ironx/pub_camera_image/lowerbody_detection', 10)
        ...
        #print("~/" + os.path.join('src', package_name,package_name) + '/haarcascade_frontallowerbody_default.xml')
            ...
            if len(point_lowerbody) > 0:
                for point in point_lowerbody:
                    ...
                    if (point_x != 0) and (point_x != 0) :
                        ...
                        Area_lowerbody=abs(Area_lowerbody)#Area_Condition-Area_lowerbody
                        #print(Area_lowerbody)
                        # Set statement for check offset left or right
                        # from 10 pixel of center
                        if dist_x>30:
                            # text are use following format
                            # 'left or Right , distance offset of aruco in pixel unit', area of lowerbody in pixel^2 unit
                            text='left,'+str(100+dist_x)+','+str(Area_lowerbody)
                            cv2.putText(frame, text , (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)

                        elif dist_x<(-30):
                            text='right,'+str(dist_x-100)+','+str(Area_lowerbody)
                            cv2.putText(frame, text, (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)

                        # Set statement for check forward and backward
                        elif abs(dist_x)< 30  and Area_lowerbody>self.UpperBound_Area_Condition:
                            # text are use following format
                            # forward or backward , area or lowerbody
                            text='backward,'+str(50)+','+str(Area_lowerbody)
                            cv2.putText(frame, text, (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)

                        elif abs(dist_x)< 30 and Area_lowerbody<self.LowerBound_Area_Condition:
                            text='forward,'+str(50)+','+str(Area_lowerbody)
                            cv2.putText(frame, text, (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)
                        
                        else :
                            text='stop,'+'0'+','+str(Area_lowerbody)
                            cv2.putText(frame, text, (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)
                ...
                msg.data = text
                self.lowerbody_detection_result_publisher.publish(msg)
            #cv2.imshow("gray",lowerbodys)
            cv2.imshow("ROS2 publisher webcam",frame)
            self.vdo_lowerbody_detection_publisher.publish(self.br.cv2_to_imgmsg(frame))
