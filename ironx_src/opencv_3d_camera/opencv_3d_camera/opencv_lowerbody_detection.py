...
class ImagePublisher(Node):
    ...
    def __init__(self):
        ...
        self.vdo_lowerbody_detection_publisher = self.create_publisher(Image, 'ironx/pub_3Dcamera_image/lowerbody_detection', 10)
        self.lowerbody_detection_result_publisher = self.create_publisher(String, 'ironx/pub_msgs/result', 10)
        ...
        self.lowerbody_cascade = cv2.CascadeClassifier(os.path.join(pkg_path,package_name) + '/haarcascade_lowerbody.xml')
    ...
    def timer_callback(self):
        ...
        #use haarcascades model for detecting lowerbody
        lowerbodys = self.lowerbody_cascade.detectMultiScale(gray_image, 1.01, 1)

        #create list for collect point of lowerbody detecrted
        point_lowerbody=[]
        # Collect point form every lowerbody im image
        for (x,y,w,h) in lowerbodys:
            area = w * h

            if area > 10000:
                # Create rectangle in lowerbody dtected
        ...
        if len(point_lowerbody) > 0:
            for point in point_lowerbody:
                ...
                distance_lowerbody = depth_frame.get_distance(point_x,point_y)
                distance_lowerbody=int(distance_lowerbody*1000)
                ...
                if (point_x != 0) and (point_y != 0) :
                    ...
                    distance_lowerbody=abs(distance_lowerbody)
                    #print(Distance_lowerbody)
                    # Set statement for check offset left or right
                    # from 10 pixel of center
                    if dist_x>75:
                        # text are use following format
                        # 'left or Right , distance offset of aruco in pixel unit', Distance of lowerbody in pixel^2 unit
                        text='left,'+str(100+dist_x)+','+str(distance_lowerbody)
                        cv2.putText(color_image, text , (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)

                    elif dist_x<(-75):
                        text='right,'+str(dist_x-100)+','+str(distance_lowerbody)
                        cv2.putText(color_image, text, (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)

                    # Set statement for check forward and backward
                    elif abs(dist_x)< 75  and distance_lowerbody<self.LowerBound_Distance_Condition:
                        # text are use following format
                        # forward or backward , Distance or lowerbody
                        text='backward,'+str(50)+','+str(distance_lowerbody)
                        cv2.putText(color_image, text, (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)

                    elif abs(dist_x)< 75 and distance_lowerbody>self.UpperBound_Distance_Condition:
                        text='forward,'+str(50)+','+str(distance_lowerbody)
                        cv2.putText(color_image, text, (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)
                    
                    else :
                        text='stop,'+'0'+','+str(distance_lowerbody)
                        cv2.putText(color_image, text, (point_x+50,point_y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)

                    msg.data = text
                    self.lowerbody_detection_result_publisher.publish(msg)
        else:
            text='stop,'+'0'
            msg.data = text
            self.lowerbody_detection_result_publisher.publish(msg)
        ...
        self.vdo_lowerbody_detection_publisher.publish(self.br.cv2_to_imgmsg(color_image))
